...
| 어드민 세부기능정의 워크북(xlsx) ("어드민 정의서로 PRD/SB", "세부기능정의 워크북") | (3) 통합 — **어드민 워크북 분기 §5.1** |
...
### 5.1 어드민 세부기능정의 워크북 → PRD + SB ★

소스가 **어드민 세부기능정의 워크북(xlsx)** 인 경우의 통합 분기.
시트 구조: `항목(메뉴) / 세부항목 / 기능(번호 아웃라인) / 세부기능(규칙 산문) / 비고 / 진행여부 / 예시 화면 / 기능매핑`.
목표 = **누락 0**: 워크북의 모든 기능 라벨·규칙·예외가 PRD·SB에 반영되고, 이를 **기계 검증**으로 보증한다.
...
Step 2   PRD 모드(§3) 수행 — 단, Step 0.1 프로세스 입력은 **spec.json이 대체**(사용자 재질문 X)
           - Step 2.5 Mermaid 프리뷰는 그대로 무조건 (조회→필터→테이블→팝업 흐름)
           - §5 예외 처리: 체크리스트 «예외» 전수 반영 (+ 공통: 조회결과 0건, API 실패, 권한)
...
Step 4   SB: §4.4 어드민 분기 수행 (PC 고정 + 템플릿 선택 질문)
           - 화면 = 항목(메뉴) 단위 1화면 기본. 세부항목명에 팝업/모달/다이얼로그 포함 시
             별도 화면이 아니라 **.wf-sub-canvas** (모달 규칙 ★)
           - Description lvl1 = 세부항목, lvl2+ = 워크북 기능번호(1-1 등) 그대로 1:1
Step 5   SB 커버리지 검증 (★ 필수):
           python3 .../parse_admin_spec.py --id {ID} --verify {SB*.html}
           → PRD와 동일 기준. 누락 라벨 0 확인 후 완료 보고.
```

규칙:
- 검증(Step 3·5)을 건너뛴 채 저장/완료 보고 금지. verify 출력(✓/✗)을 사용자에게 그대로 보여준다.
- 워크북에 없는 동작을 임의 추가하지 않는다. 불명확하면 사용자에게 질문(비고 열 참고).
- `진행여부`·`기능매핑` 열 값이 있으면 PRD §2(범위)와 Description에 반영.
